...
## Functional Uniqueness Hotspots
...
species_ui = mammal_funrar$Ui$Ui

names(species_ui) = mammal_funrar$Ui$species

site_ui = data.frame(site_Ui = (as.matrix(subset_pres[, -c(1:3)]) %*% species_ui) /
                         site_rich,
                     GlobalID = site_coords$GlobalID)

site_ui = site_ui %>%
    verify(nrow(.) == nrow(subset_pres)) %>%
    filter(!is.na(site_Ui)) %>%
...
## Geographical Restrictedness Hotspots
