...
ggplot(site_di, aes(x = Longitude, y = Latitude, z = avg_Di)) +
    stat_summary_2d(bins = 150, color = NA) +
...
ggplot(species_status, aes(x = std_Ui, y = Ri)) +
    geom_point() +
...
divr::make_quantile_density(only_di*100) +
    labs(x = "Functional Distinctiveness*100",
         y = "Density",
         fill = NULL)
```

```{r}
divr::make_quantile_density(colMeans(mammal_di, na.rm = TRUE) *100) +
    labs(x = "Species Average Functional Distinctiveness*100",
         y = "Density",
         fill = NULL)
```
